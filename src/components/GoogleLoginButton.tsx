...
const GoogleLoginButton: React.FC<GoogleLoginButtonProps> = ({
  ...
  const handleUnconfiguredClick = () => {
    onError(
      '⚠️ Google login is not configured.\n\n' +
      'To enable Google login:\n' +
      '1. Get a Google OAuth Client ID from Google Cloud Console\n' +
      '2. Add it to .env as REACT_APP_GOOGLE_CLIENT_ID\n' +
  ...
  useEffect(() => {
    ...
    initGoogle();
  }, [onSuccess, onError, disabled, isConfigured]);
    ...
    <div className="w-full">
      ...
      {(disabled || !isConfigured) && (
        <button
          ...
          disabled={disabled}
          className={`w-full py-2.5 px-4 rounded-lg font-medium border flex items-center justify-center transition-all ${
            !isConfigured
