...
import { formatRelativeTime, truncateText, createSafeDate } from "../utils";
...
interface UserListProps {
  ...
  onConversationAdd?: (conversation: ConversationItem) => void;
...
const UserList: React.FC<UserListProps> = ({
  ...
  const handleNewChatSubmit = (email: string) => {
    ...
    // This is now handled by the onChatStarted callback
    // alert(`Contact request sent to: ${email}`);
  };

  const handleChatStarted = (conversation: ConversationItem, user: any) => {
    console.log("Chat started with user:", user);

    // Add the conversation to the list
    if (onConversationAdd) {
      onConversationAdd(conversation);
    }

    // Select the new conversation - ensure proper date handling
    const userToSelect: User = {
      id: user.id,
      name: user.displayName || user.username,
      avatar: user.profilePictureUrl,
      lastSeen: createSafeDate(user.lastSeen),
      isOnline: user.isOnline,
    };
    onUserSelect(userToSelect);
  };
  ...
  const counts = useMemo(() => {
    const usersCount = conversations.filter(
  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      if (!scrollContainer || loading || !hasMore || activeTab === "groups")
        return; // Only load more on 'all' tab
  ...
  return (
    ...
    >
      ...
      <div
        className={`bg-gradient-to-r from-emerald-800 to-green-700 text-white shadow-lg ${
          ...
          <div className="flex items-center space-x-4">
            <div className="relative">
              ...
              <div
                ...
                } ${getUserStatusColor()} rounded-full border-2 border-white shadow-md`}
              ></div>
            </div>
            <div className="flex-1 min-w-0">
              ...
              <p className="text-emerald-100 font-medium mt-0.5">Available</p>
            </div>
            <div className="flex space-x-2">
              ...
              >
                <svg
                  ...
                  } text-emerald-100 group-hover:text-white transition-colors`}
                  fill="none"
                  stroke="currentColor"
                  viewBox="0 0 24 24"
                >
            ...
            {searchQuery && (
              ...
              >
                <svg
                  className={`${
                    isMobile ? "h-4.5 w-4.5" : "h-5 w-5"
                  } text-emerald-300 group-hover:text-white transition-colors`}
                  fill="none"
                  stroke="currentColor"
                  viewBox="0 0 24 24"
                >
      ...
      <div
        className={`bg-white border-b border-green-100 ${
          isMobile ? "flex-shrink-0" : ""
        }`}
      >
        <div className="flex">
        ...
        {searchQuery && (
          <div className="px-4 py-2 bg-white border-b border-gray-100">
            <p className="text-sm text-gray-600">
              ...
              {filteredConversations.length !== 1 ? "s" : ""} for "{searchQuery}
              "
            </p>
          </div>
        )}
          ...
          <div className="bg-white rounded-xl shadow-2xl max-w-sm w-full mx-4 overflow-hidden border border-green-100">
            <div className="p-6">
              ...
              <p className="text-gray-600 mb-6">
                Are you sure you want to sign out? You'll need to select your
                account again to continue chatting.
              </p>
